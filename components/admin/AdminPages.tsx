...
export const AdminPages: React.FC<AdminPagesProps> = ({
    ...
    const [editingGymPage, setEditingGymPage] = useState(false);
    ...
    return (
        <div className="space-y-10 animate-fade-in">
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl border border-gray-100 group transition-all hover:border-gold/30">
                        <div className="flex items-center gap-8 flex-1 min-w-0">
                            <div className="w-1.5 h-1.5 bg-gold rounded-full" />
                            <div className="truncate">
                                <p className="text-[9px] font-black text-gold uppercase tracking-[0.2em] mb-1">Wellness Quarter</p>
                                <p className="text-sm font-black text-charcoal truncate">Gym Page</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4 ml-6">
                            <button
                                onClick={() => setEditingGymPage(true)}
                                className="text-[10px] font-black uppercase text-primary hover:underline"
                            >
                                Modify
                            </button>
            ...
            {/* Gym Page Editor Modal */}
            {editingGymPage && (
                <div className="fixed inset-0 z-[100] bg-charcoal/90 backdrop-blur-xl flex items-center justify-center p-6 animate-fade-in">
                    <div className="bg-white w-full max-w-4xl rounded-[3rem] p-12 shadow-2xl relative overflow-hidden overflow-y-auto max-h-[90vh]">
                        <div className="flex justify-between items-start mb-10">
                            <div>
                                <h2 className="text-3xl font-black font-serif text-charcoal">Wellness Quarter</h2>
                                <p className="text-[10px] font-black uppercase tracking-widest text-gold mt-2">Fitness & Vitality Narrative</p>
                            </div>
                            <button onClick={() => setEditingGymPage(false)} className="p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors">
                                <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                        </div>
